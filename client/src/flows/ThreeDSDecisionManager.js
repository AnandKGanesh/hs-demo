      ...
      <div className="grid grid-cols-2 gap-6 h-full">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col">
          ...
          <div className="p-6 space-y-4 overflow-y-auto flex-1">
                  ...
                  {rule.expanded && (
                    <div className="px-4 pb-4 space-y-3">
                        ...
                        <div key={condition.id} className="flex items-center gap-2">
                          {idx > 0 && (
                            <div className="flex gap-1">
                              <button
                                ...
                                className={`px-2 py-1 rounded text-xs font-medium ${condition.logicalOperator === 'AND' ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30' : 'text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700'}`}
                              >
                                AND
                              </button>
                              <button
                                onClick={() => updateCondition(rule.id, condition.id, { logicalOperator: condition.logicalOperator === 'OR' ? 'AND' : 'OR' })}
                                className={`px-2 py-1 rounded text-xs font-medium ${condition.logicalOperator === 'OR' ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30' : 'text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700'}`}
                              >
                                OR
                              </button>
                            </div>
                          ...
                          <div className="relative">
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:border-gray-400 dark:hover:border-gray-500 text-sm text-gray-900 dark:text-gray-100"
                            >
                              <span>{getFieldLabel(condition.field)}</span>
                              <ChevronDown className="w-4 h-4 text-gray-400 dark:text-gray-500" />
                            </button>
                            {showFieldDropdown === `${rule.id}-${condition.id}` && (
                              <div className="absolute top-full left-0 mt-2 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
                                {Object.entries(groupedFields).map(([category, fields]) => (
                                  <div key={category}>
                                    ...
                                    {fields.map(field => (
                                      <button key={field.id} onClick={() => handleFieldSelect(rule.id, condition.id, field.id)} className="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-900 dark:text-gray-100">
                                        {field.label}
                                      </button>
                                    ))}
                                  </div>
                          ...
                          <div className="relative">
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:border-gray-400 dark:hover:border-gray-500 text-sm"
                            >
                            ...
                            {showOperatorDropdown === `${rule.id}-${condition.id}` && (
                              <div className="absolute top-full left-0 mt-2 w-36 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg z-50">
                          ...
                          {ENUM_VALUES[condition.field] ? (
                            ...
                            <input
                              ...
                              onChange={(e) => updateCondition(rule.id, condition.id, { value: e.target.value })}
                              placeholder="Value"
                              className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm w-24 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
                            />
                          )}
                      ...
                      <button
                        ...
                        className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1 mt-2"
                      >
                      ...
                      <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <div className="relative">
                          <button
                            ...
                            className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:border-gray-400 dark:hover:border-gray-500 w-full max-w-md text-gray-900 dark:text-gray-100"
                          >
            ...
            <button
              onClick={addRule}
              className="w-full py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-600 dark:text-gray-400 hover:border-gray-400 dark:hover:border-gray-500 hover:text-gray-700 dark:hover:text-gray-300 flex items-center justify-center gap-2"
            >
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              {simulationResults.length > 0 && (
                <button onClick={resetAll} className="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 flex items-center gap-1">
